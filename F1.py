...
def convert_uktime_to_danihtime(billede):
    ...
    return danish_hour_str

def get_race_date(r_date,mydate):
...
async def scrape_matches():
    ...
    for race in races:
        ...
        race_date = get_race_date(r_date,mydate)
        embedVar = discord.Embed( color=0x9D00FF)
        add_feilds(embedVar,"Date: ",f"{race_date.date().strftime('%d-%m-%Y')}\n")
        ...
        if race_date > mydate:
            ...
            for k in table_row:
                ...
                if not billede[0] == "TV":
                    ...
                    add_feilds(embedVar,"Time: ",convert_uktime_to_danihtime(billede))
                    add_feilds(embedVar, " "," ")
                    if i % 5 == 0:
